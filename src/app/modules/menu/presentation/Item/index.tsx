import { ActionIcon, Box, Button, Card, Checkbox, Group, Radio, Stack, Text, Textarea, ThemeIcon } from "@mantine/core";
...
export default function Item({ item }: ItemProps) {
    ...
    return <Box ref={target} className="item" sx={{ position: "relative" }}>
        <Card
            ...
            <Card.Section px="sm">
                ...
                <Group my="xs">
                    <Text px="sm" fz={14} fw={700} bg='green' c="white" style={{ borderRadius: 20 }} span>
                        {formatCurrency(item.sizes.find(size => !!size.default)?.price as number)}
                    </Text>
        ...
        <Card
            ...
            >
                {item.modifiers.map(modifier => <Checkbox size="xs" key={modifier} labelPosition="left" label={modifier} value={modifier} />)}
            </Checkbox.Group>
            ...
            <Group align='center'>
                ...
                <Group h={'100%'} sx={{ flex: 1 }} position='right' spacing='sm'>
                    <ActionIcon onClick={increment} size='sm' variant='filled' color="primary">
                        <IconPlus />
                    </ActionIcon>
                    <Text ta='center' sx={(theme) => ({ width: 30, backgroundColor: "#F1EECC", borderRadius: theme.spacing.xs })}>
                        {quantity}
                    </Text>
                    <ActionIcon disabled={quantity === 0 || inCart(item.title) && quantity === 1} onClick={decrement} size='sm' variant='filled' color="primary">
                        <IconMinus />
                    </ActionIcon>
                    <Text>
                        :الكمية
